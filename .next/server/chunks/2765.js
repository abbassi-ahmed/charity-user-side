    ...
    (0,react__WEBPACK_IMPORTED_MODULE_1__.useEffect)(()=>{
        const fetchBlog = async ()=>{
            try {
                ...
                await axios__WEBPACK_IMPORTED_MODULE_6__["default"].get(`https://api.olympiquemnihla.com/blogs/find-one/${id}`).then(async (response)=>{
                    setBlog(response.data);
                    await axios__WEBPACK_IMPORTED_MODULE_6__["default"].get("https://api.olympiquemnihla.com/blogs/find-all").then((res)=>{
                        const uniquePosts = res.data.filter((post)=>post.id !== blog?.id
                        );
